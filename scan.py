...
class Plexer:
    ...
    def t_ID(self, t):
        ...
        if len(t.value) > 31:
            s = self.plextoken(t)
            print(f"{s.lineno}:{s.begpos},{s.endpos}: lexer: warn: identifier too long: '{t.value}' truncated to '{t.value[:31]}'")
            t.value = t.value[:31]
        return t
    ...
    def t_error(self, t):
        t = self.plextoken(t)
        print(f"{t.lineno}:{t.begpos},{t.begpos}: lexer: warn: invalid char: '{t.value[0]}'")
        self.lexer.skip(1)
